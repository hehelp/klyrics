...
foobar2000 视觉歌词组件：内嵌面板、透明桌面歌词、Windows 任务栏歌词。先找本地 `.lrc`，再按首选项里使用中的源搜索（[LRCLIB](https://lrclib.net)、酷狗、QQ 音乐、网易云；内嵌歌词可加入为备用源）。支持行级卡拉 OK 和打轴编辑。英文显示名 **Klyrics**。
...
- **搜词**：本地 `.lrc` → 使用中的源（LRCLIB / 酷狗 / QQ / 网易云；内嵌歌词可加入为备用）；手动搜索可预览再选用。搜到的词可保存为 `.lrc` 或写入音乐标签
- **搜图**：iTunes 等来源；与搜词分开
...
2. 保存目录下 `lyrics/<歌手>/`
3. 设置里的额外路径
4. 使用中的搜索源（默认 LRCLIB / 酷狗 / QQ / 网易云）。内嵌歌词默认在可用源，加入使用中后可当备用

搜索页可选保存到 `.lrc` 或写入音乐标签；面板右键「把歌词写入音乐文件」随时写当前词。外置 CUE 写到镜像音频的分轨字段，不改 `.cue` 文本。
