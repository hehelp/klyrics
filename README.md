...
foobar2000 视觉歌词组件：内嵌面板、透明桌面歌词、Windows 任务栏歌词。先读文件标签里的歌词，没有再找本地 `.lrc`，再没有则向 [LRCLIB](https://lrclib.net)、酷狗、QQ 音乐和网易云搜索。支持行级卡拉 OK 和打轴编辑。英文显示名 **Klyrics**。

当前组件文件版本：Windows `2.26.8.16`，macOS `2.26.8.19`。English: [README_en.md](README_en.md)。
...
- **面板**：跟播滚动、卡拉 OK 高亮、封面/歌手图、双语成对显示
- **桌面歌词**：平时透明，悬停出底栏；可选横排/竖排、进度条、KTV 描边、3D 阴影、图像填充
- **浮动窗口**：独立无边框窗，平时全透明，悬停出工具栏和进度条
- **任务栏歌词**（仅 Windows）：贴在任务栏空位上的一行歌词
- **搜词**：标签 → 本地 `.lrc` → 在线（LRCLIB / 酷狗 / QQ / 网易云）；手动搜索可预览再选用
- **搜图**：iTunes 等来源；与搜词分开
...
打开歌曲后按顺序找，都没有再按首选项勾选的源自动搜（合并后取最好的一条，不弹窗）：

1. 文件标签（带 `[mm:ss]` 当 LRC，否则当纯文本）
2. 歌曲所在目录
3. 保存目录下 `lyrics/<歌手>/`
4. 设置里的额外路径

标签有词时下次仍先用标签，不会把在线结果写进音频文件。
