...
当前组件文件版本：Windows `2.26.9.3`，macOS `2.26.9.3`。产品版 `2.0.0.5`。English: [README_en.md](README_en.md)。
...
- **搜词**：本地 `.lrc` 与「使用中」的源（出厂含网易云 / 酷狗 / QQ 逐字脚本，再 LRCLIB / 酷狗 / QQ / 网易云 / 内嵌歌词）；未加入的源不搜。手动搜索可预览再选用。搜到的词可保存为 `.lrc`（含歌曲所在目录）或写入音乐标签
- **搜图**：iTunes 等来源；与搜词分开
