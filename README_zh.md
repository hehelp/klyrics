[English](README.md) · **中文**

# 快乐歌词（foo_klyrics）

foobar2000 视觉歌词组件：内嵌面板、透明桌面歌词、Windows 任务栏歌词。歌词来自本地 `.lrc` / `.ttml` 和首选项「使用中」的源。出厂先走网易云 / 酷狗 / QQ **逐字**社区脚本，再 [LRCLIB](https://lrclib.net)、酷狗、QQ、网易云逐行源；内嵌歌词、amlldb TTML、YouTube 字幕默认在可用源。未加入的源不会搜。支持标准 LRC、Enhanced LRC（行内 `<>` 逐字时间）和 Apple / AMLL 歌词 TTML（注音、译文、罗马音可右键勾选）；卡拉 OK 有字戳则按字形裁到当前字。英文显示名 **Klyrics**。

当前组件文件版本：Windows `2.26.9.20`，macOS `2.26.9.20`。产品版 `2.0.0.18`。

本仓库只托管**编译包**和开源的社区 JS 脚本，不公开插件源码。安装包在仓库的 **Releases** 页。

## 更新

### 2.0.0.18（2026-09-19）

- Windows：桌面歌词和浮窗增加「使用 CPU 渲染歌词」。勾选后改用软件渲染，默认关闭
- 保存歌词的文件名支持 `%filename%` 宏，取歌曲文件名，不含目录和扩展名

### 2.0.0.17（2026-09-17）

- 默认搜索源顺序调整为：内嵌、本地、网易云、LRCLIB，其余在可用源
- 歌词搜索结果窗口中点击「设置」按钮直接跳转至首选项「搜索 → 搜索源」页
- 桌面歌词横排与竖排模式分别记忆窗口位置和尺寸，切换时各自恢复（Windows / macOS）
- 桌面歌词按字号自动适配窗口尺寸算法优化（按约 15 个汉字阅读宽度自适应）
- 面板与浮窗：支持单独按下 Alt 键拖动时整行吸附跳播（Windows / macOS）
- D2D 歌词元素：鼠标滚轮改为仅浏览歌词、不调整播放进度，静止数秒后平滑回位
- D2D 歌词元素：快速拖动并松开鼠标时平滑保持 seek 预览，并在底层 PlaybackHub 锁定目标进度，消除因播放器异步回调产生的歌词回跳与画面滞后
- Windows 组件新增原生 ARM64EC 架构支持，官方 `.fb2k-component` 包包含 Win32、x64、ARM64EC 与 macOS 四端二进制

### 2.0.0.16（2026-09-15）

- 增加可选 Zero Bus 歌词服务（服务名 `plugin.klyrics`，需安装 [foo_zero_bus](https://github.com/hehelp/foo_zero_bus)）；偏好「Zero Bus 服务」页可开关并记日志
- 移除本机 WebSocket（`127.0.0.1:9999`）及偏好「Websocket服务」页；浏览器 Demo 改走 Zero Bus
- 只有从浮窗打开偏好页时才临时取消浮窗置顶，桌面和任务栏保持置顶

### 2.0.0.15（2026-09-14）

- 面板歌词的字号不再限制最小值 12 号
- 修复：退出 foobar 时极小概率闪退（搜索歌词 / WebSocket 线程未停净）
- 修复：偏好页-桌面，个别控件位置重叠

### 2.0.0.14（2026-09-13）

- 修复：32 位 foobar 暗黑模式下打开「快乐歌词」偏好页会把整窗切成亮色
- 修复：32 位关于页（快乐歌词根节点）背景和文字不跟随暗黑
- 浮窗 / 桌面歌词打开偏好页时不再把偏好窗设为置顶，改为自己取消置顶；关掉偏好页后再恢复

### 2.0.0.13（2026-09-13）

- 修复：播放中关闭本机 WebSocket 并点应用，界面可能卡住、下一曲无法开始
- WebSocket 设置独立为偏好「Websocket服务」页；勾选立即开听，去掉勾选立即停听
- 增加「启动服务 / 停止服务」：按是否正在监听启用或禁用；停止不改勾选
- 端口被占用时在端口框下提示错误；监听成功或手动停止也会显示状态

### 2.0.0.12（2026-09-13）

- 桌面歌词行间距可调；成对显示时两行间距跟随该设置
- 歌词服务增加接口命令：静默搜词、选歌词 / 编辑窗、图层、保存、偏好页、桌面 / 浮窗 / 任务栏显示与锁定、跳转进度、面板样式
- COM 内嵌类型库，兼容 JSplitter 3.8+
- Windows / macOS 增加本机 WebSocket 服务（默认 `127.0.0.1:9999`，偏好「更新」页可关或改端口）
- 提供了使用WebSocket歌词客户端 `klyrics_client.js,` 并用这个客户端实现了绘制歌词的demo(klyrics.js和klyrics.html)

### 2.0.0.11（2026-09-12）

- 桌面歌词支持了新的对齐方式和切行效果
- 把偏好页的页签改成了偏好树里的独立子页
- 「面板」改名为「面板模板」，只作为新建面板时的默认外观

### 2.0.0.10（2026-09-12）

- 修复2.0.0.9版本的bug: 32位版本，fb2k 2.x版本CUI界面找不到歌词面板
- 首次安装时的默认值有误，已修复为套用 Classic Dark 主题
- 在偏好页，歌词改为挂在"Tools"节点，不再挂载在"Display"节点
- 现在OBS(或基于OBS开发的抖音直播伴侣等直播软件)能捕获桌面歌词窗口和浮动歌词窗口
- 图片页只控制搜索，是否画背景图由各窗口自己的背景模式决定
- 面板 / 浮窗背景增加「图片」：可用自动搜索的专辑封面或歌手照片，或指定一张图；显示方式为居中、拉伸、填充、平铺
- 搜索页增加「过滤」：标题或专辑名包含列表中的文字则跳过自动搜词（忽略大小写；专辑为空时不按专辑过滤；手动搜索不受影响）
- 面板和浮窗增加了2种新特效: 扇形、拨盘

### 2.0.0.9（2026-09-11）

- 增加在线翻译歌词的功能，自动把搜索到的歌词翻译成目标语言
- 其他组件和 JScript Panel 可通过 COM（`Klyrics.Engine`）拉取当前歌词（仅 Windows）
- 两个推送：加载成功（行数、歌词文件路径；内嵌或未保存到文件时路径为空）；播放切行时推当前行（行号、时间、原文、译文）

